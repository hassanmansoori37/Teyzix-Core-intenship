...
const App: React.FC = () => {
  ...
  const renderPage = () => {
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16 }}>
          {[
            ...
            <div key={s.label} style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:16 }}>
              ...
              <p style={{ fontSize:22, fontWeight:500, color:t.textHead, fontFamily:"monospace", marginBottom:4 }}>{s.value}</p>
    ...
    if (activePage === "Reports") return (
      <PageShell title="Reports" desc="Generated reports ready for download" t={t}>
        {[
          ...
          <div key={r.name} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:"14px 16px", marginBottom:10 }}>
            <div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:16 }}>
          {[
            ...
            <div key={s.label} style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderTop:`2px solid ${s.color}`, borderRadius:8, padding:16 }}>
              <p style={{ fontSize:11, color:t.textMuted, marginBottom:6 }}>{s.label}</p>
              <p style={{ fontSize:24, fontWeight:500, color:t.textHead, fontFamily:"monospace" }}>{s.value}</p>
            </div>
          ))}
        </div>
      </PageShell>
    ...
    if (activePage === "Orders") return (
      <PageShell title="Orders" desc="Manage and track all customer orders" t={t}>
        {[
          ...
          <div key={o.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:"12px 16px", marginBottom:10 }}>
            <span style={{ fontSize:12, fontFamily:"monospace", color:t.textMuted }}>{o.id}</span>
            <span style={{ fontSize:13, fontWeight:500, color:t.textHead, flex:1, marginLeft:16 }}>{o.customer}</span>
            <span style={{ fontSize:13, fontFamily:"monospace", color:t.textHead, marginRight:16 }}>{o.amount}</span>
    ...
    if (activePage === "Revenue") return (
      <PageShell title="Revenue" desc="Revenue streams and forecasts" t={t}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16 }}>
          {[
            { label:"This Month",  value:"$4.82M", delta:"+18.4%", up:true  },
            { label:"Last Month",  value:"$4.07M", delta:"+12.1%", up:true  },
            { label:"This Quarter",value:"$13.4M", delta:"+21.3%", up:true  },
            { label:"Forecast",    value:"$5.1M",  delta:"+5.8%",  up:true  },
          ].map(s => (
            <div key={s.label} style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:16 }}>
              <p style={{ fontSize:11, color:t.textMuted, marginBottom:6 }}>{s.label}</p>
              <p style={{ fontSize:22, fontWeight:500, color:t.textHead, fontFamily:"monospace", marginBottom:4 }}>{s.value}</p>
        ...
        {[
          ...
          <div key={g.goal} style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:16, marginBottom:10 }}>
            ...
            <div style={{ background:t.bgInput, borderRadius:99, height:6 }}>
              <div style={{ width:`${g.progress}%`, background:g.color, height:6, borderRadius:99, transition:"width 0.5s" }} />
            </div>
          </div>
        ))}
      </PageShell>
      ...
      <PageShell title="Settings" desc="Account and application preferences" t={t}>
        <div style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:8, padding:20 }}>
          {[
            ...
            <div key={s.label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", paddingBottom: i < 2 ? 16 : 0, marginBottom: i < 2 ? 16 : 0, borderBottom: i < 2 ? `1px solid ${t.border}` : "none" }}>
              <div>
  ...
  return (
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", background:t.bgPage, color:t.textMain, fontFamily:"system-ui,sans-serif", transition:"background 0.25s, color 0.25s" }}>

      {/* ── SIDEBAR ── */}
      <aside style={{ width:210, flexShrink:0, background:t.bgSidebar, borderRight:`1px solid ${t.border}`, display:"flex", flexDirection:"column", transition:"background 0.25s" }}>
        {/* Logo */}
              ...
              {group.items.map(item => {
                ...
                return (
                  <button key={item.label} onClick={() => setActivePage(item.label)}
                    ...
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.background="rgba(59,130,246,0.08)"; e.currentTarget.style.color=t.textMain; }}
      ...
      <div style={{ display:"flex", flexDirection:"column", flex:1, minWidth:0, overflow:"hidden" }}>
        {/* Topbar */}
        <header style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 20px", borderBottom:`1px solid ${t.border}`, background:t.bgSidebar, flexShrink:0, transition:"background 0.25s" }}>
          <div style={{ flex:1 }}>
            <h1 style={{ fontSize:15, fontWeight:500, color:t.textHead, margin:0 }}>Business Intelligence Overview</h1>
            <p style={{ fontSize:11, color:t.textMuted, margin:0 }}>Last sync: 2 min ago</p>
          </div>
          ...
          <button
            ...
            style={{ display:"flex", alignItems:"center", gap:6, background: isDark ? "#1E3A5F" : "#DBEAFE", border:`1px solid ${isDark ? "#3B82F6" : "#93C5FD"}`, borderRadius:6, padding:"6px 14px", fontSize:12, color: isDark ? "#93C5FD" : "#1D4ED8", cursor:"pointer", fontFamily:"inherit", fontWeight:500, transition:"all 0.2s" }}
          >
